...
### Branch Cleanup with Autosquash

For feature branches with multiple commits, use autosquash workflow:

1. **Initial commit**: Use proper conventional commit format
   ```bash
   git commit -m "feat: add user authentication system"
   ```

2. **Follow-up commits**: Use `fixup!` or `squash!` prefixes
   ```bash
   git commit -m "fixup! feat: add user authentication system"
   git commit -m "squash! feat: add user authentication system"
   ```
